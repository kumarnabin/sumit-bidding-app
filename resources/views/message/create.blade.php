                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('messages.store') }}">
                                ...
                                <div class="col-md-6">
                                    <select id="sender_id" class="form-control @error('sender_id') is-invalid @enderror"
                                        ...
                                        @foreach($users as $sender)
                                            <option
                                                value="{{$sender->id}}" {{$sender->id==old('sender_id')?'selected':''}}>{{$sender->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <select id="receiver_id"
                                        ...
                                        @foreach($users as $receiver)
                                            <option
                                                value="{{$receiver->id}}" {{$receiver->id==old('receiver_id')?'selected':''}}>{{$receiver->name}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="message" type="text" value="{{old('message')}}"
