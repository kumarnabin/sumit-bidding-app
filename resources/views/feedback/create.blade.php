                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('feedbacks.store') }}">
                                ...
                                <div class="col-md-6">
                                    <select id="item_id" class="form-control @error('item_id') is-invalid @enderror"
                                        ...
                                        @foreach($items as $item)
                                            <option
                                                value="{{$item->id}}" {{$item->id==old('item_id')?'selected':''}}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <select id="user_id" class="form-control @error('user_id') is-invalid @enderror"
                                        ...
                                        @foreach($users as $user)
                                            <option
                                                value="{{$user->id}}" {{$user->id==old('user_id')?'selected':''}}>{{$user->name}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="rating" type="number" min="1" max="5" value="{{old('rating')}}"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="comment" type="text" value="{{old('comment')}}"
